...
import org.casualmiracles.finance.contracts.{Contract, PR}
...
class ContractsTracer extends Tracer {
  ...
  def trace(i: Int, c: Contract): Unit = if (tracing) {
    output("\t" * i); outputln(c)
  }

  def trace[T](i: Int, s: String, pr: PR[T]): PR[T] = {
    if (tracing) {
      output("\t" * i); outputln(i, s); outputln(formatPr(pr, tracehorizon, "\t" * i))
    }
    pr
  }
}